...
const NAV_LINKS = [
  ...
  { href: "/impact", label: "ESG Badges", icon: Award },
...
const CHAINS: Array<{ id: SupportedChain; name: string; icon: string; color: string }> = [
  { id: "genlayer", name: "GenLayer", icon: "🧠", color: "text-purple-400" },
  { id: "base", name: "Base", icon: "🔵", color: "text-blue-400" },
  { id: "zksync", name: "zkSync", icon: "⚡", color: "text-emerald-400" },
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-[#080B10]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        
        {/* Left: Logo */}
        <div className="flex items-center gap-6">
          <Link href="/" className="group flex items-center gap-3 shrink-0">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white p-1.5 shadow-md shadow-white/10 group-hover:scale-105 transition-transform">
            ...
            <div className="flex flex-col">
              <span className="text-lg font-bold tracking-tight text-white flex items-center gap-1.5 leading-none">
                KRIDGE
                <span className="inline-flex items-center rounded-md bg-purple-500/15 px-1.5 py-0.5 text-[9px] font-mono font-medium text-purple-300 ring-1 ring-inset ring-purple-500/30">
                  GenLayer
                </span>
              </span>
              <span className="text-[9px] font-mono text-zinc-400 tracking-wider mt-0.5">AI CREDIT MARKET</span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-white/5 border border-white/10 p-1 rounded-full backdrop-blur-md">
            {NAV_LINKS.map((link) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-3.5 w-3.5 shrink-0 ${isActive ? "text-cyan-400" : link.highlight ? "text-cyan-400" : ""}`} />
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-zinc-200 hover:bg-white/10 hover:border-cyan-500/40 transition-colors"
            >
            ...
            {chainMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 rounded-2xl border border-white/10 bg-[#0E131F] p-1.5 shadow-2xl backdrop-blur-2xl z-50 animate-in fade-in zoom-in-95 duration-150">
          ...
          {/* Wallet State Pill */}
          <div className="flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-950/30 px-3.5 py-1.5 text-xs font-mono text-cyan-300 shadow-sm shadow-cyan-500/10">
