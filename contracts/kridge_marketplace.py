...
class KridgeMarketplace(gl.contract.Contract):
    ...
    def verify_listing_health(self, listing_id: int) -> str:
        assert u256(listing_id) in self.listings, "Listing not found"
        listing = json.loads(self.listings[u256(listing_id)])
        ...
        self.listings[u256(listing_id)] = json.dumps(listing)
    ...
    def complete_rental(self, rental_id: int) -> str:
        assert u256(rental_id) in self.rentals, "Rental session not found"
        rental = json.loads(self.rentals[u256(rental_id)])
        assert rental.get("status") == "ACTIVE", "Rental is not active"

        rental["status"] = "COMPLETED"
        self.rentals[u256(rental_id)] = json.dumps(rental)

        lid = rental.get("listing_id")
        if u256(lid) in self.listings:
            listing = json.loads(self.listings[u256(lid)])
            listing["status"] = "COMPLETED"
            self.listings[u256(lid)] = json.dumps(listing)
    ...
    def resolve_dispute(self, dispute_id: int) -> str:
        assert u256(dispute_id) in self.disputes, "Dispute not found"
        dispute = json.loads(self.disputes[u256(dispute_id)])
        assert dispute.get("status") == "PENDING", "Dispute already resolved"

        rental = json.loads(self.rentals[u256(dispute["rental_id"])])
        listing = json.loads(self.listings[u256(rental["listing_id"])])
...
"""
        ...
        self.disputes[u256(dispute_id)] = json.dumps(dispute)
        self.rentals[u256(dispute["rental_id"])] = json.dumps(rental)
    ...
    def get_listing(self, listing_id: int) -> str:
        return self.listings.get(u256(listing_id), "{}")

    @gl.public.view
    def get_rental(self, rental_id: int) -> str:
        return self.rentals.get(u256(rental_id), "{}")

    @gl.public.view
    def get_dispute(self, dispute_id: int) -> str:
        return self.disputes.get(u256(dispute_id), "{}")
